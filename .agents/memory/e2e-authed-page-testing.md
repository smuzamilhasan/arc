...
- **Playwright's bundled Chromium fails in the NixOS container** with `libglib-2.0.so.0` not found. Preferred fix: point Playwright at the Replit-provided browser via `launchOptions.executablePath = process.env.REPLIT_PLAYWRIGHT_CHROMIUM_EXECUTABLE` (already wired conditionally into `playwright.config.ts`). Alternative: install the system libs (libglib, nss, gtk3, mesa/libgbm, xorg.*) via `installSystemDependencies`.
