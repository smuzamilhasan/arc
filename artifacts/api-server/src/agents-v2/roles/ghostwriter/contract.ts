...
function buildSystemPrompt(input: GhostwriterInput): string {
  ...
  return [
    ...
    `  - The body is FINISHED, PUBLISHABLE prose. NEVER put citation markers like`,
    `    "[sample 21]", "[story 29]", or "[ref 3]" in the body — those IDs go ONLY`,
    `    in the voice_evidence object, never in the visible text.`,
