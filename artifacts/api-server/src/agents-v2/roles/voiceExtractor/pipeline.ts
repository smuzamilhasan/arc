...
function renderStoryPrompt(input: VoiceExtractorInput): string {
  const stratified = stratifySamples(input.samples, 28);
  return [
    `Find redeployable FIRST-PERSON anecdotes where the user is the protagonist.`,
    ``,
    `Be willing to look closely — many users bury their own stories inside posts`,
    `that are mostly about other things (a product launch, a lesson, an industry`,
    `take). If a post contains even one or two sentences where the user describes`,
    `something THEY personally did, built, decided, lived, or learned, capture it.`,
    ``,
    `INCLUDE (the user is the subject — the protagonist):`,
    `  - "I built / I launched / I decided / I learned / I failed at / I left / when I was at X..."`,
    `  - The user's own origin, struggle, milestone, turning point, or hard call`,
    `  - A behind-the-scenes moment of something the user made or shipped`,
    `  - A realization the user reached through their own direct experience`,
    ``,
    `EXCLUDE (someone else is the protagonist):`,
    `  - "A founder overcame X" / "Wali transitioned to Y" / "my guest did Z"`,
    `  - Posts that are PURELY profiles/features/congratulations of other people`,
    `  - Generic advice with no personal anchor`,
    ``,
    `Decision test: is the user the PROTAGONIST of this moment, or the narrator of`,
    `someone else's? Capture only where the user is the protagonist — but DO capture`,
    `those even when they're embedded in a post about something else.`,
    ``,
    `It is fine to return 3-8 stories if they genuinely exist. Only return an empty`,
    `array if there are truly no first-person moments anywhere in the samples.`,
    ``,
    `Samples:`,
...
function renderReferencePrompt(input: VoiceExtractorInput): string {
  ...
  return [
    `Find the user's RECURRING intellectual anchors — the references they return to as part of how they think.`,
    ``,
    `IMPORTANT: include BOTH concrete and abstract references. Do not collapse`,
    `everything into abstract concepts. Concrete named people, books, and`,
    `frameworks are the MOST valuable — they're what make the user's writing`,
    `sound recognizably like them. Aim for a mix:`,
    `  - named PEOPLE the user repeatedly cites as influences or sparring partners`,
    `    (thinkers, founders, authors — not one-time guests)`,
    `  - specific BOOKS or FRAMEWORKS the user builds arguments on`,
    `  - recurring CONCEPTS the user has made their own`,
    ``,
    `INCLUDE a reference if it is EITHER:`,
    `  - cited across 2+ different samples, OR`,
    `  - a clearly load-bearing influence (a thinker/book/framework the user`,
    `    builds arguments on, even if named in one rich passage)`,
    ``,
    `EXCLUDE:`,
    ...
    ``,
    `Aim for 6-12 high-signal references with a healthy mix of concrete (people,`,
    `books) and abstract (concepts). If the user genuinely cites specific people`,
    `as influences, those MUST appear — don't reduce them to a generic concept.`,
    ``,
    `Samples:`,
