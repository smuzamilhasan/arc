...
- The conversational Planner agent (nav item next to Manager/Strategist; `pages/planner.tsx`, `routes/plannerChat.ts`, `services/plannerChat.ts`, `plannerMessagesTable`) OWNS ALL calendar/scheduling changes via confirm-before-apply proposals: generate_calendar, schedule_posts, reschedule_posts, delete_posts, shift_posts. Its chat/action infra mirrors the Strategist (`/planner/chat/*`: messages, unread, seen, message, actions confirm/reject + batch). The Strategist hands planning off to the Planner and the Manager RELAYS any planning brief to the Planner (persists an unseen planner_message, surfaces "Review and confirm in the Planner") rather than applying calendar changes itself — calendar writes happen in exactly one place. Pure date helpers live in `services/scheduleMath.ts` (rescheduleToDay, shiftDateByDays). The original one-shot PlannerDialog flow above is unchanged.
- Verified email is the source of truth for resolving a PERSONAL profile to its owner. `client_profile.verifiedEmail` (canonical lowercased Clerk-verified email; stamped on create, lazily backfilled) lets `reconcilePersonalProfileByEmail` (`services/inviteBinding.ts`) re-point a profile to a second Clerk identity that shares the same verified email, so a returning user who signs in under a different identity (e.g. Google vs email/password) lands on their existing profile instead of being treated as new. Matches only the caller's own verified emails and only non-agency profiles; changes ownership without overwriting content. `entry.tsx` only routes to onboarding on a confirmed 404 (no profile) and retries transient/auth errors so a momentary failure never forces onboarding.
...
- Transactional email goes through Resend via the Replit connector proxy (`@replit/connectors-sdk`). Generic helper: `services/email.ts` (`sendEmail` — never throws, returns a boolean). Invite-specific content + app-origin/invite-link derivation: `services/inviteEmail.ts`. From address defaults to Resend's shared `onboarding@resend.dev` (override with `RESEND_FROM` once a custom domain is verified). `POST /agency/:agencyId/invite` sends the invite email after persisting the row and returns `emailSent` so the UI can fall back to "Copy invite link" on failure; email failure never blocks invite creation.
